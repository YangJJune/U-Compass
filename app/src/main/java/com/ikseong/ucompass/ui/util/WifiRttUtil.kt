...
object WifiRttUtil {
    ...
    private var lastRttAccessPointCount: Int = 0
    
    /**
     * 마지막으로 측정된 RTT 액세스 포인트 개수를 반환합니다.
     * @return RTT 액세스 포인트 개수
     */
    fun getLastRttAccessPointCount(): Int {
        return lastRttAccessPointCount
    }
    ...
    private fun calculatePositionFromRttScan(
        ...
        try {
            ...
            for (scan in scanResults) {
                ...
                val x = estimatedDistance * cos(angle)
                val y = estimatedDistance * sin(angle)
            ...
            val referenceAp = validResults.maxByOrNull { result ->
                apEstimatedPositions[result.macAddress.toString()]?.third ?: Int.MIN_VALUE
            }
            ...
            // 기준 AP 정보
            val refMac = referenceAp.macAddress.toString()
